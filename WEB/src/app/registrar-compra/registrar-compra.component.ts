...
export class RegistrarCompraComponent {
  ...
  proveedorSeleccionadoId: number = 0; // Variable para almacenar el ID de la categoría seleccionada
  ...
  onProveedorSeleccionado(event: Event) {
    ...
    const proveedorSelec = element.value;
    console.log('nombreNegocio seleccionado:', proveedorSelec); // Agrega esta línea
    const proveedorEncontrado = this.proveedores.find(clientes => clientes.nombre === proveedorSelec);
    console.log('vendedorEncontrado:', proveedorEncontrado); // Agrega esta línea
    if (proveedorEncontrado) {
      this.proveedorSeleccionadoId = proveedorEncontrado.id;
      console.log('proveedorSeleccionadoId:', this.proveedorSeleccionadoId); // Agrega esta línea
    }
  }
  ...
  guardarProducto() {
    ...
    this.producto.arrayProductos.push({
      intIdVendedor: this.proveedorSeleccionadoId,
  ...
  guardarCompra(){
    const formData = {
      intIdProveedor: this.proveedorSeleccionadoId,
      floatTotalCompra: this.totalVenta,
      strStatus: 'Activo',
    ...
    this.http.post<any>('http://localhost:9005/sandec/nuevaCompra', formData, { headers }).subscribe(
      (response) => {
        ...
        this.notifier.notify('success', 'La compra se guardó correctamente.');
      ...
      (error) => {
        this.notifier.notify('success', 'La compra tuvo un error.');
